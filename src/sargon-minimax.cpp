...
"variation (PV) in the models above. Sargon itself doesn't explicitly calculate\n"
"the PV, it just concerns itself with calculating the best move (it was a\n"
"commercial game, not a modern chess engine). But by understanding its tree\n"
"structure and minimax algorithm, we can calculate the PV by saving the 'best so\n"
"far' nodes as Sargon encounters them in an ordered list external to Sargon, and\n"
"then working backwards once Sargon has finished. Using our simple 2-2-2 model as\n"
"an example, here is the list of nodes in the order that they are created;\n"
"\n"
"AGA\n"
"AGB\n"
"AG\n"
...
"B\n"
"\n"
"To calculate the PV we start by marking each node that's the 'best move so far'\n"
"as the minimax algorithm runs. We'll use an asterisk in our lists and diagrams.\n"
"\n"
"In the tree diagrams, lists of moves available in one position are represented\n"
...
"B\n"
"\n"
"(Actually Sargon performs an optimisation that reduces the number of asterisked\n"
"nodes in practice - for example it knows that BGA cannot be part of the PV\n"
"unless BGA scores better than A*, so it doesn't asterisk it unless it does. But\n"
"this doesn't affect our reasoning here and can be ignored as an optimization\n"
"detail).\n"
"\n"
"We calculate the PV by working backwards through the list of asterisked nodes.\n"
"If the minimum 7 nodes are asterisked, then the PV is A,AG,AGA. Why? A because B\n"
"was not better than A (wasn't asterisked), AG because AH wsn't better than AG\n"
"(wasn't asterisked), AGA because AHA wasn't better than AGA (wasn't asterisked).\n"
"\n"
"The PV algorithm is not hard to discern; Work backwards from the end of the list\n"
"of asterisked nodes. Find the last level one node to be asterisked (it's A*\n"
"because B wasn't asterisked). Continue to work backwards from that point looking\n"
"for a level two asterisked node (it's AG* because AH wasn't asterisked). During\n"
"that search we were only looking for level two asterisked nodes, so we quite\n"
"rightly skipped over the level three asterisked AHA* node. The AHA* node lost\n"
"it's chance to be part of the PV when AH did not trump AG and get an asterisk.\n"
"We're on the home stretch now, continue working backwards from AG* looking for a\n"
"level three asterisked node, and find it in AGA*. So solution is A,AG,AGA.\n"
"\n"
"Diagramming it;\n"
"\n"
"AGA *   <---- 3\n"
...
"B branch of the tree, (BGB trumps BGA, BHB trumps BHA, BH trumps BG) none of\n"
"this affects the PV because B fails to trump A and so the PV algorithm\n"
"rightfully skips the whole B branch searching for A* when N=1.\n"
"\n"
...
"B *     <---- 1\n"
"\n"
"This time there's a lot of activity on both branches. In fact almost all the\n"
"nodes are asterisked. B trumps A. But BH does not trump BG, so it's B,BG.\n"
"Finally BGB does trump BGA so PV = B,BG,BGB. This time the PV algorithm skips\n"
"the whole A branch because the algorithm is futilely searching for a level N=4\n"
"node as it scans backward through the A items in the list. Of course a simple\n"
"optimisation could prevent that wasted effort if necessary.\n"
"\n"
"Fortunately none of this is effected in the slightest by Alpha-Beta\n"
"optimisation, which just filters out nodes that were never going to be part of\n"
"the PV anyway.\n";
