...
MVEMSG_2 EQU     0342h
         DW      0
BRDPOS   EQU     0344h                  ; Index into the board array
         DB      1       DUP (?)
ANBDPS   EQU     0345h                  ; Additional index required for ANALYS
         DB      1       DUP (?)
LINECT   EQU     0346h                  ; Current line number
         DB      0
;**** TEMP TODO END
...
ADJPTR:  MOV     bx,word ptr [ebp+MLLST] ; Get list pointer
         MOV     dx,-MOVSIZ             ; Size of a move entry
...
PF20:    MOV     bx,NPINS               ; Address of pinned piece count
         ...
         MOV     byte ptr [ebp+esi+PLISTD],cl ; Save direction of pin  ;PLISTD_IDX
;X p46
         MOV     al,byte ptr [ebp+M4]   ; Position of pinned piece
         MOV     byte ptr [ebp+esi+PLIST],al ; Save in list ;PLISTA_IDX
...
BOOK:    POP     eax                    ; Abort return to FNDMOV
         ...
         MOV     bx,BMOVES-PTRSIZ       ; Init best move ptr to book
         MOV     word ptr [ebp+BESTM],bx
...
CONVRT:  PUSH    ecx                    ; Save registers
         ...
         lahf
         PUSH    eax
         MOV     al,byte ptr [ebp+BRDPOS] ; Get board index
...
EXECMV:  PUSH    esi                    ; Save registers
         ...
         TEST    dh,40h                 ; Double move ?
         JZ      EX14                   ; No - jump
         MOV     dx,MOVSIZ              ; Move list entry width
